...
class MonoBERT(Reranker):
    ...
    def get_model(pretrained_model_name_or_path: # str = 'models/bert-large-uncased', 
                  str = 'castorini/monobert-large-msmarco',
                  *args, device: str = None, **kwargs) -> AutoModelForSequenceClassification:
        device = device or ('cuda' if torch.cuda.is_available() else 'cpu')
        device = torch.device(device)
        print("Going to use the model", pretrained_model_name_or_path)
    ...
    def rerank(self, query: Query, texts: List[Text]) -> List[Text]:
        ...
        for text in texts:
            try:
                text.text = json.loads(text.text)['contents']
            except:
                text.text = text.text
            ret = self.tokenizer.encode_plus(query.text,
                                             ...
                                             return_tensors='pt')
            # print(self.tokenizer.convert_ids_to_tokens(ret['input_ids'][0]))
